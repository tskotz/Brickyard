...
/**
 * A 16 bit unsigned integer data element.
 */
public class UInt16Chunk extends BaseChunk
{
    private int value;

    /**
     * Creates a "unsigned" short datum. Since Java does not support unsigned shorts, we will be using a Java
     * int with an explicit range to store the "unsigned short" value.
     *
     *      C++ ->  An unsigned short is 0 to 65536 [ 2 bytes (16 bits) ]
     *      Java -> Does not support unsigned data types. Will store as a Java int. [ 4 bytes (32 bits) ]
     *
     * @param name The name of the data element, or null.
     * @param value The value of the data element.
     * @throws Exception Thrown if the value does not fall in the range of 0 to 65535
     */
    public UInt16Chunk(String name, int value) throws Exception
    {
        super(name, NotificationType.UI16);
        // Throw if we accidentally attempt to create a uint 16 datum with a value C++ can't handle
        if (value < 0 || value > 65535)
            throw new Exception(value + " out of range (0 to 65535)");
        this.value = value;
    }

    /**
     * Creates a unsigned short datum from a String. Since Java does not support unsigned shorts, we will be
     * using a Java int with an explicit range to store the "unsigned short" value.
     *
     *      C++ -> An unsigned short is -326768 to 32767 [ 2 bytes (16 bits) ]
     *      Java -> Does not support unsigned data types. Will store as a Java int. [ 4 bytes (32 bits) ]
     *
     * @param name The name of the data element, or null.
     ...
     * @throws Exception
     */
    public UInt16Chunk(String name, String digits) throws Exception
    {
     ...
     * @throws Exception
     */
    public UInt16Chunk(byte[] b, int offset, boolean swapBytes) throws Exception
    {
        super("", NotificationType.UI16);
    	...
    	if (this.mChunkSize < 10)
    		throw new Exception("Buffer to small to be an UInt16Datum");

        this.value = convertByte2ToChar(b, offset+8, swapBytes);
	...
	public byte[] toByteArray(boolean swapBytes)
    {
        ...
        byte[] datumSize = convertIntToByte4(this.mChunkSize, swapBytes);
        byte[] data = convertShortToByte2(this.value, swapBytes);
